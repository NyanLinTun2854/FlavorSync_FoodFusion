<div class="space-y-2">
    <label for="{{ $field }}" class="block text-sm font-medium text-gray-900 dark:text-white">{{ $title }}
        @if($isMandatory) <span class="text-destructive">*</span> @endif
    </label>
    <textarea id="{{ $field }}" rows="4" name="{{ $field }}"
        ...
        placeholder="{{ $placeholder ?? null }}"></textarea>
</div>